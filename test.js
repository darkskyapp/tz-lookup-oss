...
function check(lat, lon, opt1, opt2, offset1, offset2) {
  it(
    ...
    function() {
      ...
      // timezone offset lookup from location
      var offset = tz.getOffset(lat, lon)
      ...
      // timezone offset lookup from string
      if(opt1 && offset1)
        assert.equal(tz.getOffset(opt1), offset1)
      
      if(opt2 && offset2)
        assert.equal(tz.getOffset(opt2), offset2)
    }
  )
}
