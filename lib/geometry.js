...
(function() {
  ...
  Geometry = {
    ...
    length: {
      line: function(line) {
        return Math.sqrt(Geometry.lengthSquared.line(line));
      }
    },
    reverse: {
      line: function(line) {
      ...
      point: {
        line: function(point, line) {
          ...
          if(x === 0.0 && y === 0.0) {
            ...
            u = ((point[0] - line[0]) * x + (point[1] - line[1]) * y) /
                  (x * x + y * y);
        ...
        polygon: function(point, poly) {
          ...
          for(i = poly.length; i; ) {
            ...
            distance = Math.min(
              distance,
              Geometry.distanceSquared.point.line(point, line)
            );
          }

          return inside ? 0.0 : distance;
        }
      },
      line: {
        point: function(line, point) {
          return Geometry.distanceSquared.point.line(point, line);
        }
      },
      polygon: {
        point: function(poly, point) {
          return Geometry.distanceSquared.point.polygon(point, poly);
        }
      }
    },
    distance: {
      point: {
        line: function(point, line) {
          return Math.sqrt(Geometry.distanceSquared.point.line(point, line));
        },
        polygon: function(point, poly) {
          return Math.sqrt(Geometry.distanceSquared.point.polygon(point, poly));
        }
      },
      line: {
        point: function(line, point) {
          return Geometry.distance.point.line(point, line);
        }
      },
      polygon: {
        point: function(poly, point) {
          return Geometry.distance.point.polygon(point, poly);
        }
      }
    }
  };
